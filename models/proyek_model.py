...
class ProyekModel:
    ...
    @staticmethod
    def update_proyek(proyek_id: int, nama_proyek: str, klien: str,
                      deadline: str, status: str) -> bool:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute(
    ...
    def delete_proyek(proyek_id: int) -> bool:
        """Hapus proyek beserta semua tugasnya (manual cascade)."""
        conn = get_connection()
        cursor = conn.cursor()
        # Hapus tugas dulu, baru proyek (untuk DB lama yang tanpa CASCADE)
        cursor.execute("DELETE FROM tugas WHERE proyek_id = ?", (proyek_id,))
